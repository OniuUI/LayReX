...
- **`OrchestrationStreamKind.TurnResultSummary`** — end of turn; `Attributes` include `totalInputTokens`, `totalOutputTokens`, `assistantText`, `appendedCount`. These totals should match the last `ModelRoundCompleted` cumulative values when the connector reports usage consistently.
